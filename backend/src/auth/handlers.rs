use std::sync::Arc;
use crate::auth::repository::create_audit_log;
use crate::{
    app::AppState,
    auth::dto::{RegisterRequest, User},
    auth::password::hash_password,
    auth::repository::{create_user, user_exists},
...
pub async fn register(
    ...
    // Generate email verification token, store in Redis (24hr TTL)
    ...
    // Send verification email via Resend (non-critical: log, don't fail)
    ...
    // Write audit log (non-critical: log, don't fail)
    let ip_address = headers
        .get("x-forward-for")
        .or_else(|| headers.get("x-real-ip"))
        .and_then(|v| v.to_str().ok());

    if let Err(e) = create_audit_log(&app.db, new_user.id, "user.registered", ip_address).await {
        tracing::error!("Failed to write audit log: {:?}", e);
    };

    // Return 201 with user object
    Ok((StatusCode::CREATED, Json(ApiResponse::ok(new_user))))
}
